import { useState } from 'react'
import { LogIn, LogOut, Loader2 } from 'lucide-react'

const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:8000'

export default function AuthButton({ accessToken, userEmail, onAuth, onLogout }) {
  const [loading, setLoading] = useState(false)

  const handleSignIn = () => {
    // Redirect to backend OAuth initiation endpoint
    window.location.href = `${API_URL}/api/auth/google`
  }

  if (accessToken && userEmail) {
    return (
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2 bg-navy-800/60 border border-white/10 rounded-xl px-3 py-2">
          <div className="w-6 h-6 rounded-full bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-amber-400 text-xs font-bold">
            {userEmail[0]?.toUpperCase()}
          </div>
          <span className="text-white/70 text-sm">{userEmail}</span>
        </div>
        <button
          onClick={onLogout}
          className="ghost-btn flex items-center gap-1.5 text-sm py-2 px-3"
          title="Sign out"
        >
          <LogOut className="w-3.5 h-3.5" />
          Sign out
        </button>
      </div>
    )
  }

  return (
    <button
      onClick={handleSignIn}
      disabled={loading}
      className="ghost-btn flex items-center gap-2 text-sm"
    >
      {loading ? (
        <Loader2 className="w-4 h-4 animate-spin" />
      ) : (
        <svg className="w-4 h-4" viewBox="0 0 24 24" aria-hidden="true">
          <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
          <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
          <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
          <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
        </svg>
      )}
      Sign in with Google
    </button>
  )
}
